...
def border_core2instance(border_core, pred_border_core_tmp_filepath, processes=None, progressbar=True, dtype=np.uint16, reuse=False):
    """
    ...
    if not reuse:
        ...
        del props[0]
        with open(pred_border_core_tmp_filepath[:-9] + "_props.pkl", 'wb') as handle:
        ...
        instances = np.zeros_like(border_core, dtype=dtype)
        num_instances = 0
        with open(pred_border_core_tmp_filepath[:-9] + "_props.pkl", 'rb') as handle:
    ...
    if processes is None:
        ...
        instances_patches = ptqdm(border_core_component2instance, border_core_patches, processes, desc="Border-Core2Instance", disable=not progressbar, dtype=dtype)
